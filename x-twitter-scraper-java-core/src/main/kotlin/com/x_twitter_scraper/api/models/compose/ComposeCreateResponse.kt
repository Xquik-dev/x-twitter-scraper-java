...
private constructor(
    ...
    private constructor(
        ...
        @JvmSynthetic
        internal fun validity(): Int =
            (contentRules.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
                (engagementMultipliers.asKnown().getOrNull()?.sumOf { it.validity().toInt() }
                    ?: 0) +
                ...
                (radarRecommendations.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
                (scorerWeights.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
                (if (source.asKnown().isPresent) 1 else 0) +
                (topPenalties.asKnown().getOrNull()?.size ?: 0) +
                (savedStyles.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
    ...
    private constructor(
        ...
        internal fun validity(): Int =
            (compositionGuidance.asKnown().getOrNull()?.size ?: 0) +
                (examplePatterns.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
                (if (intentUrl.asKnown().isPresent) 1 else 0) +
                (if (nextStep.asKnown().isPresent) 1 else 0)
    ...
    private constructor(
        ...
        @JvmSynthetic
        internal fun validity(): Int =
            (checklist.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
                (if (nextStep.asKnown().isPresent) 1 else 0) +
